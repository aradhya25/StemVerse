...
export default function QuizHistoryPage() {
  ...
  const handleViewDetails = (attemptId) => {
    alert(`Loading full details and answer log audit for attempt: ID ${attemptId}`);
  };
  // 3. Filter and Sorting Engine
  const filteredAttempts = attempts
    .filter((att) => {
      const matchSearch = att.quiz_title.toLowerCase().includes(search.toLowerCase()) ||
      ...
      <div className="space-y-8 relative">
        ...
        {!isLoading && error && (
          <div className="bg-white rounded-3xl p-12 border border-slate-100 shadow-premium text-center space-y-6 max-w-md mx-auto">
            ...
            <div className="space-y-2">
              <h3 className="text-base font-extrabold text-darkGray">Connection Error</h3>
              <p className="text-xs text-slate-400">Unable to load your quiz attempt history from the server.</p>
            </div>
            <button
        ...
        {!isLoading && !error && (
          <div className="space-y-8 animate-fadeIn">
            ...
            {attempts.length > 0 && (
              <QuizHistoryStats attempts={attempts} />
            )}
