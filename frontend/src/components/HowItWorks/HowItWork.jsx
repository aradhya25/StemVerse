      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20 space-y-4">
          ...
          <p className="text-base text-darkGray-light font-normal">
            Learn step by step through interactive lessons, AI-powered quizzes,
            and personalized guidance designed for your success.
          </p>
        </div>
          ...
          <div className="grid grid-cols-7 gap-4 relative z-10">
            {steps.map((item, index) => (
              ...
              >
                {/* Step circle */}
                <div
                  className={`w-14 h-14 rounded-full flex items-center justify-center ${item.color} shadow-lg shadow-black/10 group-hover:scale-110 transition-transform duration-300 relative border-4 border-white`}
                >
                  {item.icon}
        ...
        <div className="lg:hidden relative space-y-8 pl-8 before:content-[''] before:absolute before:left-[1.75rem] before:top-4 before:bottom-4 before:w-[2px] before:bg-slate-200">
          {steps.map((item, index) => (
            ...
            >
              {/* Step circle */}
              <div
                className={`flex-shrink-0 w-14 h-14 rounded-full flex items-center justify-center ${item.color} shadow-md group-hover:scale-105 transition-transform duration-300 relative z-10 border-2 border-white`}
              >
                {item.icon}
              ...
              <div className="pt-2 space-y-1">
                ...
                <p className="text-sm text-darkGray-light">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
